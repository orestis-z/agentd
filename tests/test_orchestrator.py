...
def test_instance_name():
    assert _instance_name("eval-model") == "eval-model"
    assert _instance_name("fix flaky test") == "fix-flaky-test"
    assert _instance_name("my_task") == "my-task"


def test_pod_name():
    with patch.dict(os.environ, {"USER": "alice"}):
        assert pod_name("eval-model") == "devenv-alice-eval-model"


def test_pod_name_uppercase_user():
    with patch.dict(os.environ, {"USER": "Alice"}):
        assert pod_name("eval") == "devenv-alice-eval"
